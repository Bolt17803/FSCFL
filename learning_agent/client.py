...
class Client:
    def __init__(self, model_class, optimizer, local_dataset, test_dataset, use_scheduler, batch_size,
        ...
        sample_x, _ = self.local_dataset[0]
        self.input_shape = tuple(sample_x.shape)
    ...
    def compute_functional_embedding(self, shared_inputs, device):
        self.local_model.eval()
        with torch.no_grad():
            x = shared_inputs.to(device)
            ...
            embedding = torch.cat((mean_prob, var_prob), dim=0)
